...
Here's one way: for each denomination, we can use it once, or twice, or...
as many times as it takes to reach or overshoot the amount with coins of that denomination alone.

For each of those choices of how many times to include coins of each denomination,
we're left with the subproblem of seeing how many ways we can get the remaining amount from the remaining denominations.
...
It takes O(n*m)O(n∗m) time and O(n*m)O(n∗m) space, where nn is the size of amount and m is the number of items in denominations.
(Except we'd need to remove the line where we log "checking ways to make..." because making all those subarrays will take O(m^2)O(m​2) space!)

However, we can do better. Because our function is recursive it will build up a large call stack ↴ of size O(m)O(m).
Of course, this cost is eclipsed by the memory cost of memo, which is O(n*m)O(n∗m).
...
So if we presuppose that we'll use one 2¢ coin, that leaves us with 5-2=35−2=3 left to come up with.
We already know how many ways we can get 3¢ with 1¢ and 2¢ coins: waysOfDoingNCents1And2[3], which is 22.

So we can see that:

  waysOfDoingNCents1And2[5] = waysOfDoingNCents1[5] + waysOfDoingNCents1And2[5-2]

Why don't we also need to check waysOfDoingNCents1And2[5 - 2 - 2] (two 2¢ coins)?
Because we already checked waysOfDoingNCents1And2[1] when calulating waysOfDoingNCents1And2[3].
We'd be counting some arrangements multiple times.
...
We're only interested in how many more possibilities we might get when we go from kk to k+2k+2 and
thus have the ability to add one more 2¢ coin to each of the possibilities we have for kk.

===== SOLUTION =====
We use a bottom-up ↴ algorithm to build up a table waysOfDoingNCents such that
waysOfDoingNCents[k] is how many ways we can get to k cents using our denominations.
...
===== COMPLEXITY =====
O(n*m)O(n∗m) time and O(n)O(n) additional space, where n is the amount of money and m is the number of potential denominations.
...
You're taking that idea of "keeping track of what we need in order to update the best answer so far,"
and applying it to situations where the new best answer so far might not just have to do with the previous answer,
but some other earlier answer as well.

So as you can see in this problem, we kept track of all of our previous answers to smaller versions of the problem (called "subproblems")
in a big array called waysOfDoingNCents.

Again, same idea of keeping track of what we need in order to update the answer as we go,
like we did when storing the max product of 2, min product of 2, etc in the highest product of 3 question.
Except now the thing we need to keep track of is all our previous answers, which we're keeping in an array.

We built that array bottom-up, but we also talked about how we could do it top-down and memoize.
Going bottom-up is cleaner and usually more efficient, but often it's easier to think of the top-down version first and try to adapt from there.
...
*/
//ex. amount = 5 cents, denominations = [1,3,5] cent coins
// this is using bottom up approach
function cashier (amount, denominations) {
  // initialize array of zeros with indices up to amount
  var waysOfDoingNcents = [];
  for (var i = 0; i <= amount; i++) {
    waysOfDoingNcents[i] = 0;
  }
  ...
  denominations.forEach(function (coin) { // loop through type of coins
    for (var higherAmount = coin; higherAmount <= amount; higherAmount++) { // loop through amount comparing each type of coin to each amount
      var higherAmountRemainder = higherAmount - coin;
      waysOfDoingNcents[higherAmount] += waysOfDoingNcents[higherAmountRemainder];
    }
  })
