...
With our approach, we’ll put in two of the second type of cake for a total value of 400 pounds.
But we could have put in a hundred of the first type of cake, for a total value of 3000 pounds!
...
It’s pretty similar. Again we’ll track a max value, let’s say with a variable maxValueAtCapacity2.
But now we care about cakes that weigh 1 or 2 kg. What do we do with each cake?
And keep in mind, we can lean on the code we used to get the max value at weight capacity 1 kg.

If the cake weighs 2 kg, it would fill up our whole capacity if we just took one.
So we just need to see if the cake's value is higher than our current maxValueAtCapacity2.
If the cake weighs 1 kg, we could take one, and we'd still have 1 kg of capacity left.
How do we know the best way to fill that extra capacity? We can use the max value at capacity 1!
We’ll see if adding the cake's value to the max value at capacity 1 is better than our current maxValueAtCapacity2.
...
And just like we did with maxValueAtCapacity1 and maxValueAtCapacity2, now we’ll track maxValueAtCapacity3 and loop through every cake:
...
There’s gotta be a pattern here. We can keep building up to higher and higher capacities until we reach our input capacity.
Because the max value we can carry at each capacity is calculated using the max values at previous capacities,
we'll need to solve the max value for every capacity from 0 up to our duffel bag's actual weight capacity.
...
We could use an object ↴ , where the keys represent capacities and the values represent the max possible monetary values at those capacities.
Or we could use an array, where the indices represent capacities and the values represent max monetary values.
...
Let's allocate a variable currentMaxValue that holds the highest value we can carry at the current capacity.
We can start it at zero, and as we go through all the cakes, any time the value using a cake is higher than currentMaxValue, we'll update currentMaxValue!
...
We'll also need to make sure we set currentMaxValue to zero in the right place in our loops—we want it to reset every time we start a new capacity.
...
How can we handle duffel bags that can’t hold anything and cakes that weigh nothing?
...
For the first option the advantage is we get the behavior of infinity. Compared to any other integer, Infinity will be greater.
And it's a number, which can be an advantage or disadvantage—we might want our result to always be the same type,
but without manually checking we won't know if we mean an actual value or the special case of infinity.

For the second option the advantage is we can create a custom behavior that we—or our function's users—could know to expect.
The disadvantage is we'd have to explicitly check for that behavior, otherwise we might end up trying to parse the string "infinity" as an integer,
which could give us an error or (perhaps worse) a random number.
In a production system, a function that sometimes returns an integer and sometimes returns a string would probably be seen as sloppy.

The third option is a good choice if we decide infinity is usually an "unacceptable" answer.
For example, we might decide an infinite answer means we've probably entered our inputs wrong.
Then, if we really wanted to "accept" an infinite answer, we could always "catch" this exception when we call our function.
...
var maxDuffelBagValue = function (cakeTypes, weightCapacity) {
  var maxValuesAtCapacities = [];
  for (var i=0; i < weightCapacity; i++) {
    maxValuesAtCapacities[i] = 0; // store the maxValues for each weight capacity, ex. for capacity = 20  -> [0, 0, 0, ... ]
  }

  for (var currentCapacity=0; currentCapacity <= weightCapacity; currentCapacity++) { // loop through weightCapacity [0,1,2, ... 20]
    var currentMaxValue = 0; // set a variable to hold the max monetary value so far for currentCapacity
    for (var j=0; j < cakeTypes.length; j++) { // loop through all cakeTypes objects
      var cakeType = cakeTypes[j]
      if (cakeType.weight === 0 && cakeType.value !== 0) { // account for edge case
        return Infinity;
      }

      if (cakeType.weight <= currentCapacity) { // if a cake weighs as much or less than current weight capacity
        var maxValueUsingCake = cakeType.value + maxValuesAtCapacities[currentCapacity - cakeType.weight]; 
        currentMaxValue = Math.max(maxValueUsingCake, currentMaxValue);
      }
    }
